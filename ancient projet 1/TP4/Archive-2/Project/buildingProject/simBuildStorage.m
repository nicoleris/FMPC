...
refCost = [];
for i=1:size(refDist,2)
    if ( (mod(i,24*3)<=10*3) || (mod(i,24*3)>=16*3) )
        refCost = [refCost 0.2];
    else
        refCost = [refCost 0.04];
    end
end
refSetbacks = [];
for i=1:size(refDist,2)
    if ( (mod(i,24*3)<=8*3) || (mod(i,24*3)>=18*3) )
        refSetbacks = [refSetbacks 4];
    else
        refSetbacks = [refSetbacks 0];
    end
end


for i = 1:T
[d_pred, cp, sb] = fhandle(i, N, refCost, refSetbacks);
