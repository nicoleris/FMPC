...
% *) We observe that the  iterates k = 5, 100, 200, 300 do not
% satisfy the constraints of the system (The state sequences go out of the
% states’ constraint set). This is normal because only z_bar satisfies the
% constraints. However, for higher iterates (k >> 300 for ex), when
% z_bar - z converges to 0, the state sequences satisfies the constraint
% and it stays in the states' constraint set.
 
% *) If we set xmax to 9. The problem becomes unfeasible using YALMIP
% (results.problem == 1). On the contrary of YALMIP, ADMM generates a
% solution that does not satisfy the constraints of the system.
% In some cases where we have flexible constraints, ADMM has the advantages
% to generate a solution in all the cases. However, this can be a
% disadvantage if we have a system with strict constraints.
